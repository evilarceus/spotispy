  ...
  getJson(config, function(data) {
    try {
      if (data.track.track_resource.name !== currentTitle) {
        ...
        // Write the song artist and name to song.txt
        fs.writeFile("song.txt", data.track.artist_resource.name + " - " + data.track.track_resource.name, function(err) {
          if (err) { return console.error("Could not write to song.txt!")}
        });
...
function getAlbumCover(id) {
  ...
  getJson(config, function(data) {
    ...
    request(coverUrl).pipe(fs.createWriteStream("album.jpg")).on('close', function() {
      // do nothing
    });
